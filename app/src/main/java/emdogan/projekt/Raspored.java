...
public class Raspored extends AppCompatActivity {
    DBAdapter db;
    int rowId;
    int columnId;
    ...
    private boolean MenuChoice(MenuItem item)
    {
        ...
        ShowTimetableEntry(c.getString(2), c.getString(3),columnId,rowId);

        db.close();
        return true;
    }

    public void ShowTimetableEntry(String kraticaPredmeta, String color, int day, int when)
    {
        TableLayout tableLayout = (TableLayout)findViewById(R.id.tableLayoutId);

        TextView textView = (TextView)((TableRow)tableLayout.getChildAt(when)).getChildAt(day);

        textView.setText(kraticaPredmeta);
    ...
    public void ShowTimetable()
    {
        ...
        if (c.moveToFirst())
        {
            do {
                ...
                ShowTimetableEntry(subject.getString(2), subject.getString(3), c.getInt(1),c.getInt(2)-7);
            } while (c.moveToNext());
        }
        db.close();
    }
    ...
    public void onClickDeleteTimetable(View view) {
        AlertDialog alertDialog = new AlertDialog.Builder(Raspored.this).create();
